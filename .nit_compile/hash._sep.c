/* This C file is generated by NIT to compile module hash. */
#include "hash._sep.h"
val_t hash___Object___hash(val_t p0){
  struct {struct stack_frame_t me;} fra;
  val_t REGB0;
  val_t REGB1;
  val_t tmp;
  fra.me.prev = stack_frame_head; stack_frame_head = &fra.me;
  fra.me.file = LOCATE_hash;
  fra.me.line = 20;
  fra.me.meth = LOCATE_hash___Object___hash;
  fra.me.has_broke = 0;
  fra.me.REG_size = 1;
  fra.me.REG[0] = NIT_NULL;
  fra.me.REG[0] = p0;
  /* /Users/tony/Projects/nit/lib/standard//hash.nit:24 */
  REGB0 = CALL_kernel___Object___object_id(fra.me.REG[0])(fra.me.REG[0]);
  REGB1 = TAG_Int(8);
  /* /Users/tony/Projects/nit/lib/standard//kernel.nit:219 */
  REGB1 = TAG_Int(UNTAG_Int(REGB0)/UNTAG_Int(REGB1));
  /* /Users/tony/Projects/nit/lib/standard//hash.nit:24 */
  goto label1;
  label1: while(0);
  stack_frame_head = fra.me.prev;
  return REGB1;
}
val_t hash___Int___hash(val_t p0){
  struct {struct stack_frame_t me;} fra;
  val_t REGB0;
  val_t tmp;
  fra.me.prev = stack_frame_head; stack_frame_head = &fra.me;
  fra.me.file = LOCATE_hash;
  fra.me.line = 28;
  fra.me.meth = LOCATE_hash___Int___hash;
  fra.me.has_broke = 0;
  fra.me.REG_size = 0;
  REGB0 = p0;
  /* /Users/tony/Projects/nit/lib/standard//hash.nit:28 */
  goto label1;
  label1: while(0);
  stack_frame_head = fra.me.prev;
  return REGB0;
}
val_t hash___Char___hash(val_t p0){
  struct {struct stack_frame_t me;} fra;
  val_t REGB0;
  val_t tmp;
  fra.me.prev = stack_frame_head; stack_frame_head = &fra.me;
  fra.me.file = LOCATE_hash;
  fra.me.line = 32;
  fra.me.meth = LOCATE_hash___Char___hash;
  fra.me.has_broke = 0;
  fra.me.REG_size = 0;
  REGB0 = p0;
  /* /Users/tony/Projects/nit/lib/standard//kernel.nit:382 */
  REGB0 = TAG_Int((unsigned char)UNTAG_Char(REGB0));
  /* /Users/tony/Projects/nit/lib/standard//hash.nit:32 */
  goto label1;
  label1: while(0);
  stack_frame_head = fra.me.prev;
  return REGB0;
}
val_t hash___Bool___hash(val_t p0){
  struct {struct stack_frame_t me;} fra;
  val_t REGB0;
  val_t REGB1;
  val_t tmp;
  fra.me.prev = stack_frame_head; stack_frame_head = &fra.me;
  fra.me.file = LOCATE_hash;
  fra.me.line = 36;
  fra.me.meth = LOCATE_hash___Bool___hash;
  fra.me.has_broke = 0;
  fra.me.REG_size = 0;
  REGB0 = p0;
  /* /Users/tony/Projects/nit/lib/standard//hash.nit:38 */
  if (UNTAG_Bool(REGB0)) {
    /* /Users/tony/Projects/nit/lib/standard//hash.nit:39 */
    REGB0 = TAG_Int(1);
    goto label1;
  } else {
    /* /Users/tony/Projects/nit/lib/standard//hash.nit:41 */
    REGB1 = TAG_Int(0);
    REGB0 = REGB1;
    goto label1;
  }
  label1: while(0);
  stack_frame_head = fra.me.prev;
  return REGB0;
}
